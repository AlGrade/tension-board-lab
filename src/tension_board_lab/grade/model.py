...
def grade_loss(
    ...
    spreads = target_spreads.to(logits.dtype)
    grade_axis = torch.arange(logits.shape[-1], device=logits.device, dtype=logits.dtype)
    gaussian_targets = torch.exp(
        -0.5
        * (
            (grade_axis.unsqueeze(0) - ordinal_targets.unsqueeze(1))
            / spreads.clamp_min(0.1).unsqueeze(1)
        ).square()
    )
    gaussian_targets = gaussian_targets / gaussian_targets.sum(dim=-1, keepdim=True)
    soft_targets = torch.where(spreads.unsqueeze(1) > 0, gaussian_targets, soft_targets)
