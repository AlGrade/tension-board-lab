...
const fixtures = readArtifact<GeneratorFixtures>("generator_fixtures.json");
const unavailable = !board || !artifact || !fixtures;
const vocabulary = artifact ? new GeneratorVocabulary(artifact) : undefined;

describe.skipIf(unavailable)(`generator vocabulary (${MISSING_ARTIFACTS})`, () => {
...
describe.skipIf(unavailable)("prefix encoding", () => {
...
describe.skipIf(unavailable)("token sequences", () => {
...
describe.skipIf(unavailable)("constraint masks", () => {
  // Built per test, not here: vitest evaluates a skipped describe body to collect its tests,
  // so anything touching the artifacts at this level runs even when they are missing.
  const rules = () =>
    new BoulderConstraints(vocabulary!, board!, artifact!, fixtures!.layout);

  (fixtures?.mask_states ?? []).forEach((state) => {
    it(`matches Python exactly at state "${state.name}"`, () => {
      const mask = rules().maskFor(state.chosen);
  ...
  it("blocks EOS until the problem has a start and a finish", () => {
    ...
    expect(rules().isComplete(both.chosen)).toBe(true);
    expect(rules().isComplete(empty.chosen)).toBe(false);
  });
